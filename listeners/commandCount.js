...
async function incrementValueInStatsModel(id, variableToIncrement) {
	if (typeof (id) !== "string" || typeof (variableToIncrement) !== "string") {
		throw new TypeError("hey wait that's supposed to be a string!");
	}
...
class CommandListener extends Listener {
	...
	async exec(message, command) {
		...
		const {stats} = await userModel.findById(message.author.id, "stats");
		...
		await userModel.findByIdAndUpdate(message.author.id, {stats});
	}
}
